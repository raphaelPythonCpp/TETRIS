...
class Individu(object):
    def __init__(self, jeu, algoEntrainement, algo, lConstantes, nbLignes, nbColonnes):
        self.jeu = jeu
        ...
        self.nbConstantes = len(self.lConstantes) if self.lConstantes is not None else 0
        self.score = 0
        self.finJeu = False
        #Grille
    ...
    def reset(self, lConstantes=None):
        self.lConstantes = lConstantes
        self.nbConstantes = len(self.lConstantes) if self.lConstantes is not None else 0
        self.nbCoups = 0
        self.score = 0
    ...
    def jouer_coup(self):
        if self.algoEntrainement.modeCoups == 1 :
            ...
            raise KeyError("Mode coups 2 pas implémenté !")
            self.algo.joueur_2_coups(self.grille, self.lPositionsPieces, self.piece, self.holdPiece, jeu=self, lConstantes=self.lConstantes, reel=False, fScore=self.changer_score)
        elif self.algoEntrainement.modeCoups == 3:
            self.algo.jouer_3_coups(self.grille, self.lPositionsPieces, self.piece, self.holdPiece, jeu=self, lConstantes=self.lConstantes, reel=False, fScore=self.changer_score)
