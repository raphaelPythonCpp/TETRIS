...
class Menus(object):
    def __init__(self, jeu, police, horloge):
        ...
        self.menuDidactique = Menu_didactique(self.jeu, self.police, ["hauteut max grille", "hauteur max piece", "somme hauteurs", "nb trous normaux", "score irregularites", "nb lignes", "score puits"], 50, avecTorch=self.jeu.algorithme) #ATTENTION : pas le bon nom de fichier
...
class Menu_didactique(object):
    def __init__(self, jeu, police, lNomsConstantes, nbParties, avecTorch):
        self.jeu = jeu
        self.police = police
        ...
        self.generateurTexte = Generateur_Texte(self, self.dossier, f"{self.dossierImagesBase}\\CARACTERES_TEXTE", self.dossierImages, dicoNomsFichiersChiffres, transparence=True, enregistrement=False)
    ...
    def gerer_souris(self):
        ...
        if self.boutonEcriture.gerer_souris():
            self.modeTexte = "POLICE" if self.modeTexte == "IMAGES" else "IMAGES"
            for slider in self.lSliders:
    ...
    def changer_resultats(self):
        if self.modeTexte == "POLICE" :
            self.texteScoreMoyen = self.police.render(f"{self.scoreMoyen:.2f}", True, self.couleurTexte)
        elif self.modeTexte == "IMAGES":
...
class Slider(object):
    ...
    def maj_nom(self):
        ...
        if self.menu.modeTexte == "POLICE":
            ...
            self.texteNom = self.menu.generateurTexte.creer_surface_texte(self.nom, wTexte, hTexte, eT)
