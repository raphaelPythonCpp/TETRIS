...
class Algorithme(object):
    def __init__(self, jeu):
        ...
        self.lScoresMax = [self.jeu.nbLignes, self.jeu.nbLignes, self.jeu.nbColonnes*self.jeu.nbLignes, self.jeu.nbColonnes*(self.jeu.nbLignes-1), self.jeu.nbLignes*(self.jeu.nbColonnes-1), 4]
        #Positions prédites
    ...
    def calculer_toutes_positions_2_coups(self, grille, piece1, piece2):
        ...
        lMatrices1 = [self.jeu.dicoMatricesPieces[(tInit1, i)] for i in range(4)]
        while file:
            x,y,o = file.popleft()
            for dx,dy,do in dxyo :
                ...
                y2 = y+dy
                o2 = (o+do)%4
                if not self.jeu.tester_chevauchement(grille, x2-2, y2, o2, tInit1, lMatrices1[o2]) and not ((grillePositions[y2][x2] >> o2) & 1):
            ...
            for x in range(nbColonnes+4):
                for o in range(4):
                    if (grillePositions[y][x] >> o) & 1:
                        if self.jeu.tester_chevauchement(grille, x-2, y+1, o, tInit1, lMatrices1[o]):
        ...
        while file:
            ...
            for dx,dy,do in dxyo :
                ...
                y2 = y+dy
                o2 = (o+do)%4
                if not self.jeu.tester_chevauchement(grille, x2-2, y2, o2, tInit2, lMatrices2[o2]) and not ((grillePositions[y2][x2] >> o2) & 1):
                    grillePositions[y2][x2]|= (1 << o2)
                    file.append((x2,y2,o2))
        for y in range(nbLignes):
            for x in range(nbColonnes+4):
                for o in range(4):
                    if (grillePositions[y][x] >> o) & 1:
                        if self.jeu.tester_chevauchement(grille, x-2, y+1, o, tInit2, lMatrices2[o]):
    ...
    def calculer_toutes_positions_1_coup(self, grille, piece):
        ...
        while file:
            ...
            for dx,dy,do in dxyo :
                ...
                y2 = y+dy
                o2 = (o+do)%4
                if not self.jeu.tester_chevauchement(grille, x2-2, y2, o2, tInit, lMatrices[o2]) and not ((grillePositions[y2][x2] >> o2) & 1):
                    grillePositions[y2][x2] |= (1 << o2)
                    file.append((x2,y2,o2))
        lPositions = []
        for y in range(nbLignes):
            for x in range(nbColonnes+4):
                for o in range(4):
                    if (grillePositions[y][x] >> o) & 1:
                        if self.jeu.tester_chevauchement(grille, x-2, y+1, o, tInit, lMatrices[o]):
                            lPositions.append((x-2,y,o,tInit))
        #keep que les solutions valides
        lPositionsFinales = []
        for x,y,o,t in lPositions:
            if not self.jeu.tester_chevauchement(grille, x, y, o, t, lMatrices[o]):
    ...
    def hauteur_max_piece(self, x, y, o, t, nbLignesGrille):
        ...
        for dx, dy in matrice:
            dyMin = min(dy, dyMin)
        return nbLignesGrille - (y + dyMin) if dyMin < float("inf") else None
    ...
    def calculer_meilleure_position_2_coups(self, grille, lPositions):
        ...
        for iP,(x1,y1,o1,t1,x2,y2,o2,t2) in enumerate(lPositions):
            grilleTemp = [ligne.copy() for ligne in grille] #deepcopy(grille)
            self.jeu.mettre_dans_grille(grille, x1, y1, o1, t1, self.jeu.dicoCouleursPieces[t1], coin=False)
            lScores1[iP] = self.calculergrilleTemp_score(grille, x1, y1, o1, t1)
            self.jeu.mettre_dans_grille(grilleTemp, x2, y2, o2, t2, self.jeu.dicoCouleursPieces[t2], coin=False)
    ...
    def calculer_meilleure_position_1_coup(self, grille, lPositions, lConstantes=None):
        if lConstantes is None:
            nbConstantes = self.nbConstantes
            lConstantes = self.lConstantes
        else :
            nbConstantes = len(lConstantes)
        lScores = [None]*len(lPositions)
        for iP,(x, y, o, t) in enumerate(lPositions):
            grilleTemp = [ligne.copy() for ligne in grille] #deepcopy(grille)
            self.jeu.mettre_dans_grille(grilleTemp, x, y, o, t, self.jeu.dicoCouleursPieces[t], coin=False)
            nbLignesASupprimer = sum(1 for ligne in grilleTemp if all(case is not None for case in ligne))
            if nbLignesASupprimer > 0:
                self.jeu.tester_lignes(grilleTemp, reel=False, fScore=None)
            lScores[iP] = self.calculer_score(grilleTemp, x, y, o, t, nbLignesASupprimer)
            #self.jeu.enlever_de_grille(grille, x, y, o, t, None, coin=False)

        scoreMaxNormalise = -float("inf")
        meilleurePosition = (None,None,None,None) #(x,y,o,t)
        for iP,position in enumerate(lPositions):
            scoreNormalise = 0
            for i in range(nbConstantes):
                scoreNormalise += lScores[iP][i] / self.lScoresMax[i] * lConstantes[i]
            if scoreNormalise > scoreMaxNormalise :
                scoreMaxNormalise = scoreNormalise
                meilleurePosition = position
        return meilleurePosition, scoreMaxNormalise

    def calculer_score(self, grille, X, Y, O, T, nbL):
        nbLignes = len(grille)
        nbColonnes = len(grille[0])
        ...
        for x in range(nbColonnes):
            ...
            for y in range(nbLignes):
                if grille[y][x] is not None:
                    ...
                    if not est_trou :
                        if y > 0 and grille[y-1][x] is not None:
                            est_trou = True
                            nbTrous = 1
                    else :
                        nbTrous += 1
            I += abs(yMaxAvant - yMax)
            yMaxAvant = yMax
            nbTN += nbTrous
            sumHG += yMax
        hMaxP = self.hauteur_max_piece(X, Y, O, T, nbLignes)
        score = [-hMaxG, -hMaxP, -sumHG, -nbTN, -I, nbL]
    ...
    def appliquer_position(self):
        if self.nbCoups == 1:
            self.lPositionsPieces = self.joueur(self.jeu.grille, self.jeu.piece)
    ...
    def jouer_1_coup(self, grille, lPositionsPieces, piece, jeu=None, lConstantes=None, reel=True, fScore=None):
        meilleurePosition, meilleurScore = self.calculer_meilleure_position_1_coup(grille, lPositionsPieces, lConstantes)
    ...
    def joueur_3_coups(self, grille, lPositionsPieces, piece, holdPiece, jeu=None, lConstantes=None, reel=True, fScore=None):
        jeu = jeu if jeu is not None else self.jeu
        meilleurePositionPiece, scorePiece = self.calculer_meilleure_position_1_coup(grille, lPositionsPieces[0], lConstantes)
        meilleurePositionHold, scoreHold = self.calculer_meilleure_position_1_coup(grille, lPositionsPieces[1], lConstantes)
