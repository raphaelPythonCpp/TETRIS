...
class Algorithme_Genetique(object):
    def __init__(self, jeu, algo, nbIndividus, nbGenerations, nbParties, lCouches, tauxSurvivant, tauxRandom, nbLignes, nbColonnes, nbCoupsMax, modeCoups):
        self.jeu = jeu
        self.algo = algo
        
        self.nbGenerations = nbGenerations
        self.nbParties = nbParties
        self.lCouches = [] #ex : [(7, 8), (8, 1)] <=> 7 -> 8 -> ReLU -> 1
        ...
        for i in range(self.nbIndividus):
            self.lIndividus.append(Individu_NN_v1(jeu, self, algo, self.generer_reseau(self.lCouches), nbLignes, nbColonnes))
    ...
    def generer_reseau(self, lCouches, poids, biais):
        dico = {}
        iC = 0
        for i, (nbN1, nbN2) in enumerate(lCouches):
            if poids :
                limite = sqrt(6 / (nbN1 + nbN2))
                dico[f"{iC}.weight"] = torch.empty(nbN2, nbN1).uniform_(-limite, +limite)
            if biais:
                limite = sqrt(6 / (nbN1 + nbN2))
                dico[f"{iC}.bias"] = torch.empty(nbN2).uniform_(-limite, +limite)
