...
from TETRIS_algorithme_v8 import*
from TETRIS_TETROMINO_v4 import*
from TETRIS_algorithm_genetique_v4 import*
from TETRIS_NES_v1 import*
from TETRIS_DRL_v3 import*

class Jeu(object):
    def __init__(self, fenetre, police, tailleCase=50, nbColonnes=5, nbLignes = 8, visuel=True, nbFramesAffichage=1, entrainementGreedy=False, entrainementGenetique=False, entrainementNES=False, entrainementDRL=False, gameInfini=False):
        ...
        self.algoGenetique = Algorithme_Genetique(jeu=self, algo=self.algo, nbIndividus=50, nbGenerations=10, nbParties=30, nbConstantes=self.algo.nbConstantes, tauxSurvivant=0.3, tauxRandom=0.2, nbLignes=self.nbLignes, nbColonnes=self.nbColonnes, nbCoupsMax=self.nbCoupsMax, modeCoups=3)
        #NES
        ...
        self.pg = Policy_Gradient(self, self.algo, 1000, 30, self.nbCoupsMax, 1, self.algo.nbConstantes, self.nbLignes, self.nbColonnes)
        self.reset()
    ...
    def evaluation_algo(self, nbParties, lConstantes, affichage, visuel):
        tAvant = datetime.now()
        sommeScores, sommeNbCoups = 0,0
        scoreAvant, nbCoupsAvant = 0, 0
        visuelAvant = self.visuel
        self.visuel = visuel
        ...
        for iP in range(1, nbParties+1):
            if affichage:
                ...
                print(f"Evaluation Algorithme : Partie {iP} ({(iP)/nbParties*100:.2f}%)", flush=True)
    ...
    def tester_lignes(self, grille, reel=False, fScore=None):
        ...
        if nbLignesASupprimer == 0:
            return
        grille[:] = [[None]*nbColonnes for _ in range(nbLignesASupprimer)] + grilleLignesPasCompletes
        if reel :
