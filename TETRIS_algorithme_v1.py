...
class Algorithme(object):
    def __init__(self, jeu):
        ...
        #S = ...
        self.jeu = jeu
        self.alpha = 0.2 #hauteur max grille
        self.beta = 0.2 #hauteur max piece
        self.gamma = 1 #nbTrous
        self.delta = 0.2 #irregularite
        self.lConstantes = [self.alpha, self.beta, self.gamma, self.delta]
    def reset(self):
        pass
    ...
    def calculer_meilleure_position(self, grille, lPositions, hauteurMaxGrille, hauteurMaxPiece, trous, irregularites):
        ...
        for iP,(x,y,o,t) in enumerate(lPositions):
            ...
            lScores[iP] = self.calculer_score(grille, hauteurMaxGrille, hauteurMaxPiece, trous, irregularites, x, y, o, t)
            self.jeu.enlever_de_grille(grille, x, y, o, t, self.jeu.dicoCouleursPieces[t], coin=False)
        lMax = list(max(1e-20, max(abs(score[i]) for score in lScores if score[i] is not None)) for i in range(4))
        scoreMax = -float("inf")
        meilleurePosition = (None,None,None,None) #(x,y,o,t)
        for iP,(x,y,o,t) in enumerate(lPositions):
            score = sum(lScores[iP][i]/lMax[i] * self.lConstantes[i] for i in range(4))
            if score > scoreMax :
                scoreMax = score
                meilleurePosition = (x,y,o,t)
        return meilleurePosition
    
    def calculer_score(self, grille, hauteurMaxGrille, hauteurMaxPiece, trous, irregularites, x, y, o, t):
        ...
        if trous :
            score[2] = -self.nb_trous_normal(grille)
        if irregularites :
            score[3] = -self.irregularites(grille)
        return score
    ...
    def jouer(self, grille, lPositionsPieces, piece, hauteurMaxGrille, hauteurMaxPiece, trous, irregularites):
        meilleurePosition = self.calculer_meilleure_position(grille, lPositionsPieces, hauteurMaxGrille, hauteurMaxPiece, trous, irregularites)
