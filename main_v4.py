...
game_infini = True
env = Jeu(fenetre=fenetre, police=police, tailleCase=tailleCase, nbColonnes=nbColonnes, nbLignes=nbLignes, visuel=True, nbFramesAffichage=1, entrainementGreedy=False, entrainementGenetique=True)

env.changer_game_nb_coups_max(game_infini)
env.changer_somme_nb_blocs(False)
if not env.entrainementGenetique:
    env.algo.lConstantes = [0.58, 0.02, 0.83, 0.27, 0.34, 0.69]
    ...
    for iP in range(1, nbParties+1):
        ...
        scoreAvant, nbCoupsAvant = env.score, env.nbCoups
