...
import random as rd
from math import*
from TETRIS_algorithme_v6 import*
from TETRIS_TETROMINO_v2 import*
from TETRIS_algorithm_genetique_v2 import*

class Jeu(object):
    def __init__(self, fenetre, police, tailleCase=50, nbColonnes=5, nbLignes = 8, nbFramesAffichage=1, entrainementGreedy=False, entrainementGenetique=False, gameInfini=False):
        ...
        #Algo Genetique
        self.algoGenetique = Algorithme_Genetique(jeu=self, algo=self.algo, nbIndividus=10, nbGenerations=10, nbParties=10, nbConstantes=self.algo.nbConstantes, tauxSurvivant=0.3, nbLignes=self.nbLignes, nbColonnes=self.nbColonnes, nbCoupsMax=self.nbCoupsMax)
        self.reset()
        if entrainementGreedy:
            self.entrainement_greedy()
        if entrainementGenetique :
    ...
    def changer_game_nb_coups_max(self, gameInfini):
        self.nbCoupsMax = float("inf") if gameInfini else 1000
