...
class Algorithme(object):
    def __init__(self, jeu):
        ...
        self.zeta = 1 #lignes
        self.eta = 1 #puits
        self.lConstantes = [self.alpha, self.beta, self.gamma, self.delta, self.epsilon, self.zeta, self.eta]
        self.nbConstantes = len(self.lConstantes)
        self.lScoresMax = [self.jeu.nbLignes, self.jeu.nbLignes, self.jeu.nbColonnes*self.jeu.nbLignes, self.jeu.nbColonnes*(self.jeu.nbLignes-1), self.jeu.nbLignes*(self.jeu.nbColonnes-1), self.jeu.dicoScores[4], (self.jeu.nbColonnes-2)*(self.jeu.nbLignes*(self.jeu.nbLignes+1)//2)]
        #Positions prédites
    ...
    def calculer_meilleure_position_2_coups(self, grille, lPositions, lConstantes=None):
        ...
        lScoresTotal = [[None]*nbConstantes for _ in range(len(lPositions))]
        for iP,(x1,y1,o1,t1,x2,y2,o2,t2) in enumerate(lPositions):
            lScores1[iP],grilleTemp = self.caracteristiques(grille,x1,y1,o1,t1)
            lScores2[iP],grilleTemp = self.caracteristiques(grilleTemp, x2, y2, o2, t2)
        ...
        for iP,position in enumerate(lPositions):
            scoreNormalise = 0
            for i in range(nbConstantes):
                scoreNormalise += lScoresTotal[iP][i]*lConstantes[i]
    ...
    def calculer_meilleure_position_1_coup(self, grille, lPositions, lConstantes=None):
        ...
        for iP,(x, y, o, t) in enumerate(lPositions):
            lScores[iP],_ = self.caracteristiques(grille, x,y,o,t)
        ...
        for iP,position in enumerate(lPositions):
            scoreNormalise = 0
            for i in range(nbConstantes):
                scoreNormalise += lScores[iP][i]*lConstantes[i]
            if scoreNormalise > scoreMaxNormalise :
                scoreMaxNormalise = scoreNormalise
                meilleurePosition = position
        return meilleurePosition, scoreMaxNormalise

    def caracteristiques(self, grille, X, Y, O, T):
        nbLignes = len(grille)
        nbColonnes = len(grille[0])
        grilleTemp = [ligne.copy() for ligne in grille]
        self.jeu.mettre_dans_grille(grilleTemp, X, Y, O, T, (0,0,0), coin=False)
        nbL = self.nb_lignes(grilleTemp)
        if nbL > 0:
