...
class Algorithme_Genetique(object):
    def __init__(self, jeu, algo, nbIndividus, nbGenerations, nbParties, nbConstantes, tauxSurvivant, nbLignes, nbColonnes):
        self.jeu = jeu
        self.algo = algo
    ...
    def entrainement(self):
        ...
        for iG in range(self.nbGenerations):
            print(f"Entrainement Algorithme génétique : {(iG+1) / self.nbGenerations * 100}%")
...
class Individu(object):
    ...
    def generer_piece(self):
        if not self.lProchainesPieces:
            self.lProchainesPieces = self.jeu.lTypePieces.copy()
            shuffle(self.lProchainesPieces)
        self.piece = Tetromino(self.jeu, self.lProchainesPieces[-1])
